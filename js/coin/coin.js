window.onload = function () {
  const outside_reward_button = document.querySelector(".reward-button");
  outside_reward_button.addEventListener("click",viewModal);
  const body = document.getElementsByTagName("body")[0];
  body.addEventListener("click", hideModal);
  const modal_reward_main = document.querySelector(".reward-main");
  // 防止点击 modal 列表时列表消失
  modal_reward_main.addEventListener("click",(e)=>{stopProp(e);})

  function viewModal (e) {
    stopProp(e);  // 防止事件冒泡导致触发body上绑定的点击事件
    const disp = modal_reward_main.style.display;
    modal_reward_main.style.display = disp === "none" ? "flex" : "none";
  }

  function hideModal () {
    modal_reward_main.style.display = "none";
  }
  /**
   * 阻止事件冒泡
  */
